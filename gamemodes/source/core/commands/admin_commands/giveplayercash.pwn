alias:giveplayercash("givemoney", "givecash", "giveplayermoney")
CMD:giveplayercash(playerid, const params[])
{
	...
	{
		SendClientMessage(playerid, COLOUR_INFORMATION, "Gamoiyenet brdzaneba /giveplayercash [targetid] [cash]");
		return false;
	}
	...
	SetPlayerCash(targetid, (GetPlayerCash(targetid) + cash), false);

	new adminMessage[144];
	format(adminMessage, sizeof adminMessage, "%s[AdminCMD] %s[%d] - misca fuli %s[%d]-s. raodenoba:[$%d]", \
	...
	"INSERT INTO `administrator_cash_log` \
