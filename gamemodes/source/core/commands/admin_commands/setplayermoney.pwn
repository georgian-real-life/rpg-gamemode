alias:setplayermoney("setmoney", "setcash", "setplayercash")
CMD:setplayermoney(playerid, const params[])
{
	...
	{
		SendClientMessage(playerid, COLOUR_INFORMATION, "Gamoiyenet brdzaneba /setplayermoney [targetid] [cash]");
		return false;
	}
	...
	SetPlayerCash(targetid, cash, false);

	new adminMessage[144];
	format(adminMessage, sizeof adminMessage, "%s[AdminCMD] %s[%d] - sheucvala fuls raod. %s[%d]-s. raodenoba:[$%d]", \
