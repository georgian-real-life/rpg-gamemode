alias:setplayerbankmoney("setbankmoney", "setplayerbank")
CMD:setplayerbankmoney(playerid, const params[])
{
	...
	{
		SendClientMessage(playerid, COLOUR_INFORMATION, "Gamoiyenet brdzaneba /setplayerbankmoney [targetid] [cash]");
		return false;
	}
	...
	SetPlayerBankCash(targetid, cash, true);

	new adminMessage[144];
	format(adminMessage, sizeof adminMessage, "%s[AdminCMD] %s[%d] - sheucvala bankis fulis raod. %s[%d]-s. raodenoba:[$%d]", \
